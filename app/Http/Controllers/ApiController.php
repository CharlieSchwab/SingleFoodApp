...
class ApiController extends Controller {
	...
	public function getProfileUser($id) {
		...
		$json['is_partner']   		  = !empty($user->is_partner) ? $user->is_partner : 0;		
	...
	public function getRestaurant(Request $request) {
		if (!empty($this->restaurant_id)) {
			...
			if(!empty($checkRestaurant))
			{
				...
				$getPartner = User::where('restaurant_id','=',$this->restaurant_id)
									->where('status','=',0)
									->where('is_delete','=',0)
									->where('is_admin','=',3)
									->get();
				...
				$partner = array();
				foreach ($getPartner as $value_pa) {
					$data_pa = array();
					$data_pa['id'] = $value_pa->id;
					$data_pa['name'] = $value_pa->name;
					$partner[] = $data_pa;
				}

				$subjson['partner'] = $partner;
		...
		if (!empty($this->restaurant_id) && !empty($request->item_id) && !empty($this->token) && !empty($request->qty)) 
		{
			...
			if (!empty($checkRestaurant) && !empty($user_id)) 
			{
				...
				if(empty($status))
				{
					...
					$check_cart = CartModel::where('user_id','=',$user_id)->where('item_id','=',$request->item_id)->get();
					foreach ($check_cart as $key => $cart) {
						if(!empty($cart->option_id) && !empty($request->option_id))
						{
							$cart_option = explode(",", $cart->option_id);
							array_multisort($cart_option);
							$request_option = explode(",", $request->option_id);
						    array_multisort($request_option);

						    if(count($cart_option) == count($request_option))
						    {
						    	$result = array_diff_assoc($cart_option,$request_option);
							
								if(empty($result))
								{
									$main_cart_id = $cart->id;
								}
						    }
						}
						else
						{
							if(empty($request->option_id) && empty($cart->option_id))
							{
								$main_cart_id = $cart->id;
							}
						}
					...
					{
						if(!empty($main_cart_id))
						{
							$cart = CartModel::find($main_cart_id);
							$qty = $qty + $cart->qty;							
						}
						else
						{
	...
	// start order partner
	public function get_my_order_partner(Request $request)
	{
		if(!empty($this->restaurant_id) && !empty($this->token))
		{
			$user_id = $this->checkToken();
			...
			if(!empty($checkRestaurant) && !empty($user_id))
			{
				...
				$orders = OrdersModel::select('orders.*','order_assign_partner.id as assign_partner_id')
						->where('orders.is_delete', '=', 0)
						->join('order_assign_partner','order_assign_partner.order_id','orders.id')
						->where('orders.is_payment', '=', 1)
						->where('order_assign_partner.partner_id', '=', $user_id)
						->where('orders.restaurant_id', '=' ,$this->restaurant_id);

				if(!empty($request->status_id)) {
					$orders = $orders->where('order_assign_partner.status_id','=',$request->status_id);
				}
				...
				if(!empty(count($orders))) {
					...
					foreach ($orders as $value) {
						...
						$PartnerStatus = OrderAssignPartnerModel::find($value->assign_partner_id);

						$data['id'] 				= $value->assign_partner_id;
						...
						$data['timestamp'] 		    = strtotime($value->created_at);
						$data['order_status'] 		= !empty($PartnerStatus->getorderstatus->name)?$PartnerStatus->getorderstatus->name:'';
	...
	public function get_my_order_detail_partner(Request $request)
	{
		if(!empty($request->order_id) && !empty($this->restaurant_id) && !empty($this->token))
		{
			$user_id = $this->checkToken();
			...
			if(!empty($checkRestaurant) && !empty($user_id))
			{
				...
				$value = OrdersModel::select('orders.*','order_assign_partner.id as assign_partner_id')
						->join('order_assign_partner','order_assign_partner.order_id','orders.id')
						->where('order_assign_partner.partner_id', '=', $user_id)
						->where('order_assign_partner.id', '=' ,$request->order_id)
						->where('orders.restaurant_id', '=' ,$this->restaurant_id)->first();

				if(!empty($value))
				{

					$finaldata = $this->get_partner_order_single($value->assign_partner_id);
	...
	public function get_partner_order_single($assign_partner_id)
	{
		$value = OrdersModel::select('orders.*','order_assign_partner.id as assign_partner_id')
					->join('order_assign_partner','order_assign_partner.order_id','orders.id')
					->where('order_assign_partner.id', '=' ,$assign_partner_id)
					->where('orders.restaurant_id', '=' ,$this->restaurant_id)->first();


		$PartnerStatus = OrderAssignPartnerModel::find($assign_partner_id);


		$finaldata['id'] 			 = $assign_partner_id;
		$finaldata['transaction_id'] = !empty($value->transaction_id) ? $value->transaction_id : '';
		$finaldata['name'] 			 = $value->name;
		$finaldata['address_one'] 	 = $value->address_one;
		$finaldata['address_two'] 	 = $value->address_two;
		$finaldata['city'] 			 = $value->city;
		...
		$finaldata['delivery'] 		 = $this->getformatnumber($value->delivery);
		$finaldata['payment_type'] 	 = $value->payment_type;
		$finaldata['order_status'] 	 = !empty($PartnerStatus->getorderstatus->name)?$PartnerStatus->getorderstatus->name:'';
		$finaldata['status_id'] 	 = intval($PartnerStatus->status_id);
		...
		$itemdata = array();
	 	...
	 	$getstatus_p = OrderPartnerStatusModel::all();	

		$status_partner = array();
	
	 	foreach ($getstatus_p as $statusvalue_p) {

	 		$pdata['id']   = $statusvalue_p->id;
	 		$pdata['name'] = $statusvalue_p->name;
	 		$status_partner[] = $pdata;

	 	}


	 	$finaldata['status_partner'] = $status_partner;
	...
	public function change_order_status_partner(Request $request)
	{
		if(!empty($request->order_id) && !empty($this->restaurant_id) && !empty($this->token) && !empty($request->status_id))
		{
			...
			if(!empty($checkRestaurant) && !empty($user_id))
			{
				$checkorder = OrdersModel::select('orders.*','order_assign_partner.id as assign_partner_id')
						->join('order_assign_partner','order_assign_partner.order_id','orders.id')
						->where('order_assign_partner.partner_id', '=', $user_id)
						->where('order_assign_partner.id', '=' ,$request->order_id)
						->where('orders.restaurant_id', '=' ,$this->restaurant_id)->first();
				...
				if(!empty($checkorder))
				{
					$order 			  = OrderAssignPartnerModel::find($request->order_id);
					$order->status_id = $request->status_id;
					$order->save();

					$this->sendPushNotificationCutsomer($order->getorderstatus->title,$order->getorderstatus->message,$this->restaurant_id);
			...
			if(!empty($checkRestaurant) && !empty($user_id))
			{
				...
				if(!empty(count($orders))) {
					...
					foreach ($orders as $value) {
						...
						$data['order_status'] 		= !empty($value->status->name)?$value->status->name:'';
						$data['is_collection'] 		= !empty($value->is_collection) ? $value->is_collection : 0;
						$data['partner']			= $this->get_partner_status($value->id);
						$result[] = $data;
					}
	...
	public function get_partner_status($order_id)
	{
		$assign = OrderAssignPartnerModel::get_assign_order($order_id);
		$result = array();
		foreach ($assign as $value) {
			$data = array();
			$data['id'] = $value->id;
			$data['name'] = !empty($value->user) ?  $value->user->name : '';
			$data['status_id'] = $value->status_id;
			$data['status_name'] = !empty($value->getorderstatus) ? $value->getorderstatus->name : '';
			$result[] = $data;
		}
			...
			if(!empty($checkRestaurant) && !empty($user_id))
			{
				...
				if(!empty($value))
				{
					...
					$itemdata = array();
				 	...
				 	$finaldata['items']   = $itemdata;
			 		$finaldata['partner'] = $this->get_partner_status($request->order_id);
	...
	public function get_today_restaurant_detail(Request $request)
	{
		...
		if(!empty($this->restaurant_id) && !empty($this->token))
		{
			...
			if(!empty($checkRestaurant) && !empty($user_id))
			{
				$total_order = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_order = $total_order->where('added_date', '>=', $request->start_date);
							$total_order = $total_order->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_order = $total_order->where('added_date', '=', date('Y-m-d'));
						}
				$total_order = $total_order->count();



				$total_cash_order = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_cash_order = $total_cash_order->where('added_date', '>=', $request->start_date);
							$total_cash_order = $total_cash_order->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_cash_order = $total_cash_order->where('added_date', '=', date('Y-m-d'));
						}
						$total_cash_order = $total_cash_order->where('payment_type', '=', 'Cash');
						$total_cash_order = $total_cash_order->count();


				$total_card_order = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_card_order = $total_card_order->where('added_date', '>=', $request->start_date);
							$total_card_order = $total_card_order->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_card_order = $total_card_order->where('added_date', '=', date('Y-m-d'));
						}
						$total_card_order = $total_card_order->where('payment_type', '=', 'Card');
						$total_card_order = $total_card_order->count();



				$total_cash_payment = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_cash_payment = $total_cash_payment->where('added_date', '>=', $request->start_date);
							$total_cash_payment = $total_cash_payment->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_cash_payment = $total_cash_payment->where('added_date', '=', date('Y-m-d'));
						}
						$total_cash_payment = $total_cash_payment->where('payment_type', '=', 'Cash');
						$total_cash_payment = $total_cash_payment->sum('total_price');



				$total_card_payment = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_card_payment = $total_card_payment->where('added_date', '>=', $request->start_date);
							$total_card_payment = $total_card_payment->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_card_payment = $total_card_payment->where('added_date', '=', date('Y-m-d'));
						}
						$total_card_payment = $total_card_payment->where('payment_type', '=', 'Card');
						$total_card_payment = $total_card_payment->sum('total_price');


				
				$total_amount = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_amount = $total_amount->where('added_date', '>=', $request->start_date);
							$total_amount = $total_amount->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_amount = $total_amount->where('added_date', '=', date('Y-m-d'));
						}
						$total_amount = $total_amount->sum('total_price');



				$total_pending_order = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_pending_order = $total_pending_order->where('added_date', '>=', $request->start_date);
							$total_pending_order = $total_pending_order->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_pending_order = $total_pending_order->where('added_date', '=', date('Y-m-d'));
						}
						$total_pending_order = $total_pending_order->where('status_id', '=', 1);
						$total_pending_order = $total_pending_order->count();


				$total_accepted_order = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_accepted_order = $total_accepted_order->where('added_date', '>=', $request->start_date);
							$total_accepted_order = $total_accepted_order->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_accepted_order = $total_accepted_order->where('added_date', '=', date('Y-m-d'));
						}
						$total_accepted_order = $total_accepted_order->where('status_id', '=', 2);
						$total_accepted_order = $total_accepted_order->count();

				$total_processing_order = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_processing_order = $total_processing_order->where('added_date', '>=', $request->start_date);
							$total_processing_order = $total_processing_order->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_processing_order = $total_processing_order->where('added_date', '=', date('Y-m-d'));
						}
						$total_processing_order = $total_processing_order->where('status_id', '=', 3);
						$total_processing_order = $total_processing_order->count();


				
					$total_delivered_order = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_delivered_order = $total_delivered_order->where('added_date', '>=', $request->start_date);
							$total_delivered_order = $total_delivered_order->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_delivered_order = $total_delivered_order->where('added_date', '=', date('Y-m-d'));
						}
						$total_delivered_order = $total_delivered_order->where('status_id', '=', 4);
						$total_delivered_order = $total_delivered_order->count();

					$total_cancelled_order = OrdersModel::where('is_payment', '=', '1')
						->where('is_delete', '=', '0')->where('restaurant_id', '=', $this->restaurant_id);
						if(!empty($request->start_date) && !empty($request->end_date))
						{
							$total_cancelled_order = $total_cancelled_order->where('added_date', '>=', $request->start_date);
							$total_cancelled_order = $total_cancelled_order->where('added_date', '<=', $request->end_date);
						}
						else
						{
							$total_cancelled_order = $total_cancelled_order->where('added_date', '=', date('Y-m-d'));
						}
						$total_cancelled_order = $total_cancelled_order->where('status_id', '=', 5);
						$total_cancelled_order = $total_cancelled_order->count();
				...
				$total_customer = User::where('restaurant_id', '=', $this->restaurant_id)->count();
	...
	public function app_submit_support(Request $request)
	{
		if(!empty($this->restaurant_id) && !empty($this->checkToken())) 
		{
			$checkRestaurant = $this->checkRestaurant($this->restaurant_id);
			if(!empty($request->title) && !empty($request->description))
			{
				$support              = new SupportModel;
				$support->user_id     = $this->checkToken();
				...
				$json['status'] = true;
				$json['message'] = 'Your information successfully sent.';
			}
			else
			{
				$json['status'] = false;
	...
	public function app_get_support()
	{
		if(!empty($this->restaurant_id) && !empty($this->checkToken())) 
		{
			$checkRestaurant = $this->checkRestaurant($this->restaurant_id);
			...
			{
				$user_id = $this->checkToken();
				$support = SupportModel::where('user_id','=', $user_id)->orderBy('id','desc')->paginate(20);
				$result = array();
				foreach ($support as $key => $value) {
					...
					$data['status'] = intval($value->status);
					$data['timestamp'] = strtotime($value->created_at);

					$main_reply = array();
					foreach ($value->getsupportreply as $reply) {
						$data_r = array();
						$data_r['id'] = $reply->id;
						$data_r['user_id'] = $reply->user_id;
						$data_r['description'] = $reply->description;
						$data_r['timestamp'] = strtotime($reply->created_at);
						$main_reply[] = $data_r;
					}

					$data['reply'] = $main_reply;

					$result[] = $data;
				}

				$page = 0;
				if(!empty($support->nextPageUrl()))
	            {
		              $parse_url =parse_url($support->nextPageUrl()); 
		              if(!empty($parse_url['query']))
		              {
		                   parse_str($parse_url['query'], $get_array);     
		                   $page = !empty($get_array['page']) ? $get_array['page'] : 0;
	...
	public function app_support_reply(Request $request)
	{
		if(!empty($this->restaurant_id) && !empty($this->checkToken())) 
		{
			if(!empty($request->support_id) && !empty($request->description))
			{
				$support_reply             = new SupportReplyModel;
				$support_reply->support_id = $request->support_id;
				$support_reply->user_id    = $this->checkToken();
				$support_reply->description= $request->description;
				$support_reply->save();

				$json['status'] = true;
				$json['message'] = 'Your reply successfully sent.';
	...
	public function app_get_schedule(Request $request)
	{
		if(!empty($this->restaurant_id) && !empty($this->checkToken())) 
		{
			$result = array();
			$week_time_row  = WeekTimeModel::get();
			$week_time = array();

			foreach ($week_time_row as $value) {
				$data = array();
				$data['id']   = $value->id;
				$data['name'] = $value->name;
				$week_time[]  = $data;
			...
			foreach ($week as $value) {
				...
				$getuserweek =  UserTimeModel::getDetailAPI($value->id, $this->checkToken());
                $open_close  = !empty($getuserweek->status)?$getuserweek->status:'';
                $start_time  = !empty($getuserweek->start_time)?$getuserweek->start_time:'';
                $end_time    = !empty($getuserweek->end_time)?$getuserweek->end_time:'';

				$data_w = array();
				$data_w['week_id'] 	  = $value->id;
				$data_w['name'] 	  = $value->name;
				$data_w['start_time'] = $start_time;
				$data_w['end_time']   = $end_time;
				$data_w['open_close'] = !empty($open_close) ? $open_close : 0;
				$week_main[] = $data_w;
			}

			$result['week'] = $week_main;
 	...
 	public function app_submit_schedule(Request $request) {

		if(!empty($this->restaurant_id) && !empty($this->checkToken())) 
		{
			$week = json_decode($request->week);
	
			UserTimeModel::where('user_id', '=', $this->restaurant_id)->delete();
			...
			if(!empty($week))
			{
				foreach($week as $value)
				{
					if(!empty($value->open_close))
					{
						...
						$record->user_id 	= $this->restaurant_id;
	...
	public function app_assign_partner(Request $request)
	{
		if(!empty($this->restaurant_id) && !empty($this->checkToken())) 
		{
			if(!empty($request->partner_id) && !empty($request->order_id))
			{
				$count =  OrderAssignPartnerModel::where('partner_id','=',$request->partner_id)->where('order_id','=',$request->order_id)->first();
				if(!empty($count))
				{
					$order = $count;
				}
				...
				{
					$order              = new OrderAssignPartnerModel;
				...
				$json['status']  = true;
				$json['message'] = 'Your order successfully assign.';
				$json['result']  = $this->get_partner_order_single($order->id);				
			}
			else
			{
				$json['status'] = false;
